...
/**
 * Klasa sprawdza poprawność numeru NIP
 *
 ...
 */
class nipValidator extends identificationNumberValidator
{
	/**
	 * Określa, czy algorytm ma brac pod uwagę to, czy istnieje Urząd Skarbowy
	 * pasujący do kodu w numerze NIP
	 * 
	 * @var bool
	 */
	static public $CHECK_DEPARTMENT=true;
	
	/**
	 * Określa, czy w wypadku podania numeru w wersji europejskiej, algorytm ma
	 * sprawdzać, czy kod pasuje do kraju.
	 * UWAGA. W praktyce sprawdzane jest, czy pierwsze 2 znaki to 'PL', bo algorytm dotyczy
	 * polskich numerów NIP
	 * 
	 * @var bool
	 */
	static public $CHECK_COUNTRY_CODE=true;
	
	/**
	 * Tablica wag dla numeru NIP
	 *
	 * @var array
	 */
	...
	/**
	 * Zawiera tablicę ze składowymi numeruy NIPm któee powstały poprzez
	 * rozdzielenie po znaku '-'
	 * 
	 */
	protected $nipArray=array();
	
	/**
	 * Pozbawiony znaków '-' oraz pierwszych dwóch znaków numer NIP
	 * 
	 * @var string
	 */
	protected $rawNIP='';
	...
	0=>"Podany numer NIP ma nieprawdłową ilosć znaków",
	1=>"Podany numer NIP jest nieprawidłowy",
	2=>"Podany numer NIP znajduje się na liście niepoprawnych numerów",
	);

	/**
	 * Ustawia numer NIP i wywołuje sprawdzenie jego poprawności
	 *
	 * @param string $string
	 * @return void
	 */
	public function validate($string)
	{
		...
		if(preg_match("/^[A-Z]{2}[0-9\-]/", $string))
		{
			$this->countryCode=substr($string, 0, 2);
			$string=substr($string, 2);
		}

		$this->nipArray=explode("-", $string);
		$this->rawNIP=implode("", $this->nipArray);

		if(is_numeric($this->rawNIP))
		{
			if(strlen($this->rawNIP)!==10)
			{
				$this->handleError(0);
				return;
			}
	...
	protected function doCheck()
	{
		...
		for($i=0; $i<=8; ++$i)
		{
			$controlSum=$controlSum+($this->rawNIP[$i]*$this->weights[$i]);
		}
		$controlNumber=$controlSum%11;

		if($this->rawNIP[9]==$controlNumber)
		{
			if(self::$CHECK_DEPARTMENT && !$this->hasDepartment($this->nipArray[0]))
			{
				$this->handleError(2);
				$this->valid=false;
				return;
			...
			if(self::$CHECK_COUNTRY_CODE && isset($this->countryCode) && !$this->hasCountryCode($this->countryCode))
			{
				$this->handleError(3);
				$this->valid=false;
				return;
	...
	/**
	 * Zwraca nazwę Urzędu Skarbowego, który WYDAŁ numer NIP
	 * Nazwa ta nie jest tożsama z nazwą US obsługującego płatnika
	 *
	 * @param integer
	 * @return string
	 */
	public function getDepartmentName($code=false)
	{
		if(!$code)
		{
			$code=substr($this->rawNIP, 0, 3);
		}
		//print $this->rawNIP;
	...
	/**
	 * Sprawdza, czy Urząd Skarbowy o podanym kodzie istnieje
	 * 
	 * @param integer
	 * @return bool
	 */
	...
	/**
	 * Sprawdza, czy kod kraju przekazany w numerze NIP, istnieje
	 * 
	 * @param integer
	 * @return bool
	 */
	public function hasCountryCode($code)
	{
	...
	/**
	 * Zwraca nazwę kraju, w którym wydano NIP
	 * 
	 * @return string
	 */
	...
	/**
	 * Kody i nazwy Urzędów Skarbowych
	 *
	 * @var array
	 */
