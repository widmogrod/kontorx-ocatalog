<?php
class Catalog_Model_Service extends Promotor_Model_Abstract {
	protected $_dbTableClass = 'Catalog_Model_DbTable_Service';
	 ...
	 */
	protected function _selectService($serviceId) {
		$table = $this->getDbTable();
        ...
        $select
        	->distinct(true)
	        ->from(array('csc' => 'catalog_service_cost'), array())
	        ->joinInner(array('c' => 'catalog'),'csc.catalog_id = c.id')
...
//	        	$adapter->quoteInto('cs.id = ?', (int) $serviceId), array())
	        ...
	        ->where('csc.catalog_service_id = ?', $serviceId)
